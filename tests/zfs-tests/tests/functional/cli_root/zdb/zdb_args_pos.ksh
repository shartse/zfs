...
{
	typeset -a args=("-A" "-b" "-C" "-c" "-d" "-D" "-G" "-h" "-i" "-L" "-M" "-P" "-s" "-v" "-Y")
        for i in ${args[@]}; do
		log_must zdb $i $TESTPOOL
	done
}
...
function test_exported_pool
{
	...
	typeset -a args=("-A" "-b" "-C" "-c" "-d" "-D" "-F" "-G" "-h" "-i" "-L" "-M" "-P" "-s" "-v" "-X" "-Y")
        for i in ${args[@]}; do
		log_must zdb -e $i $TESTPOOL
	done
